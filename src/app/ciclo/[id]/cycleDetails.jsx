...
import { Alert, Box, Button, Card, CardContent, Container, Divider, Grid, List, ListItem, ListItemText, MenuItem, Stack, TextField, Typography } from "@mui/material";
...
export function CycleDetails({ cycle }) {
	const router = useRouter();
	const params = useParams();
	const [newName, setNewName] = useState(cycle.name || "");
	const [weeklyHours, setWeeklyHours] = useState(Number(cycle.weeklyHours));
		...
		<Box sx={{ flex: 1, display: "flex", flexDirection: "column", backgroundColor: "background.default" }}>
			<Container maxWidth="md" sx={{ py: 6 }}>
				<Stack spacing={2}>
					...
					<Typography color="text.secondary">Rota dinamica: /ciclo/{cycleId}</Typography>
					...
					{!loading && cycle ? (
						<Card>
							<CardContent>
								<Stack spacing={3}>
									<TextField label="Nome do ciclo" value={newName} onChange={event => setNewName(event.target.value)} fullWidth />
									<TextField label="Horas semanais disponiveis" type="number" value={weeklyHours} onChange={event => setWeeklyHours(Number(event.target.value))} inputProps={{ min: 1 }} fullWidth />

									<Stack spacing={2}>
										{subjects.map((subject, index) => (
											<Grid container spacing={2} key={`${subject.name}-${index}`}>
												<Grid size={{ xs: 12, md: 4 }}>
													<TextField label="Materia" value={subject.name} onChange={event => updateSubject(index, { name: event.target.value })} fullWidth />
												</Grid>
												<Grid size={{ xs: 12, md: 4 }}>
													<TextField
														select
									...
									<Stack direction={{ xs: "column", sm: "row" }} spacing={1}>
										<Button variant="contained" startIcon={<SaveIcon />} onClick={saveCycleChanges} disabled={saving}>
											{saving ? "Salvando..." : "Salvar alteracoes"}
										</Button>
										<Button variant="outlined" color="error" startIcon={<DeleteForeverIcon />} onClick={removeCycle}>
											Excluir ciclo
										</Button>
									</Stack>
									...
									<Typography color="text.secondary">Horas semanais base: {weeklyHours}h</Typography>
									...
									<List disablePadding>
										{plan.subjects?.map((subject, index) => (
											<ListItem key={`${subject.name}-${index}`} sx={{ px: 0 }}>
												<ListItemText primary={subject.name} secondary={`Afinidade ${subject.affinityRank} • Peso final ${subject.finalWeight}x • ${subject.recommendedHours}h`} />
											</ListItem>
										))}
									</List>
								</Stack>
