		...
		<Box sx={{ flex: 1, display: "flex", flexDirection: "column", backgroundColor: "background.default" }}>
			<Container maxWidth="lg" sx={{ py: 6 }}>
				<Stack spacing={1} sx={{ mb: 4 }}>
					...
					<Typography color="text.secondary">Informe as materias, afinidade e pesos extras. O sistema distribui o tempo de forma proporcional e garante minimo de 2h por materia.</Typography>
				</Stack>
				...
				<Grid container spacing={3}>
						...
						<Card>
							<CardContent>
								<Stack spacing={3}>
									...
									<TextField label="Horas semanais disponiveis" type="number" value={weeklyHours} onChange={event => setWeeklyHours(Number(event.target.value))} />
					...
					<Grid size={{ xs: 12, lg: 4 }}>
						<Stack spacing={2}>
							<Alert severity="info">Formula usada: horas = ceil((horas semanais / soma dos pesos) * peso final), com minimo de 2h por materia.</Alert>
								...
								<CardContent>
									<Stack spacing={1.5}>
										...
										{plan.subjects.map(subject => (
											<Typography key={`${subject.name}-${subject.affinityRank}-${subject.extraWeight}`}>
												{subject.name || "Materia sem nome"}: {subject.recommendedHours}h ({subject.finalWeight}x)
											</Typography>
										))}
									</Stack>
								</CardContent>
